...
def main(argv):
    ...
    elif action == "tenured_gen_usage":
        return check_tenured_gen_usage(host, port, user, passwd, memory_pool, warning, critical, perf_data)
...
def check_tenured_gen_usage(host, port, user, passwd, memory_pool, warning, critical, perf_data):
    warning = warning or 80
    critical = critical or 90
    ...
    try:
        ...
        message = "Tenured_Gen Utilization %sMB of %sMB" % (used_heap, max_heap)
        message += performance_data(perf_data, [("%.2f%%" % percent, "tenured_gen_usage", warning, critical)])
